...
static void goodix_ts_suspend(struct early_suspend *h)
{
	...
	if (ts->power) {
		ret = ts->power(ts,0);
		if (ret < 0)
			printk(&client->dev, "%s power off failed\n", f3x_ts_name);
	}
	return ;
}
...
static void goodix_ts_resume(struct early_suspend *h)
{
	...
	if (ts->power) {
		ret = ts->power(ts, 1);
		if (ret < 0)
			printk(&client->dev, "%s power on failed\n", f3x_ts_name);
	}
...
/**********************************************************************	
±¾³ÌÐòÖÐI2CÍ¨ÐÅ·½Ê½Îª£º
	7bit´Ó»úµØÖ·£ü¶ÁÐ´Î» + buf£¨Êý¾ÝµØÖ·+¶ÁÐ´Êý¾Ý£©
	 --------------------------------------------------------------------
	£ü  ´Ó»úµØÖ·   £ü buf[0](Êý¾ÝµØÖ·) | buf[1]~buf[MAX-1](Ð´Èë»ò¶ÁÈ¡µ½µÄÊý¾Ý)  |
	 --------------------------------------------------------------------
	ÒÆÖ²Ç°Çë¸ù¾Ý×ÔÉíÖ÷¿Ø¸ñÊ½ÐÞ¸Ä£¡£¡
***********************************************************************/
...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
	...
	if((1024 == screen_max_x) && (600 == screen_max_y))	{			//828
	uint8_t data_info[] = {
                            0x0F,0x80,
...
¹¦ÄÜ£º
	´¥ÃþÆÁ¹¤×÷º¯Êý
	ÓÉÖÐ¶Ï´¥·¢£¬½ÓÊÜ1×é×ø±êÊý¾Ý£¬Ð£ÑéºóÔÙ·ÖÎöÊä³ö
²ÎÊý£º
	ts:	clientË½ÓÐÊý¾Ý½á¹¹Ìå
return£º
    void
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    static u8 finger_last[MAX_FINGER_NUM+1]={0};        //ÉÏ´Î´¥Ãþ°´¼üµÄÊÖÖ¸Ë÷Òý
    u8 finger_current[MAX_FINGER_NUM+1] = {0};        //µ±Ç°´¥Ãþ°´¼üµÄÊÖÖ¸Ë÷Òý
...
¹¦ÄÜ£º
	GT80XµÄµçÔ´¹ÜÀí
²ÎÊý£º
	on:ÉèÖÃGT80XÔËÐÐÄ£Ê½£¬0Îª½øÈëSleepÄ£Ê½
return£º
	ÊÇ·ñÉèÖÃ³É¹¦£¬Ð¡ÓÚ0±íÊ¾ÉèÖÃÊ§°Ü
********************************************************/
...
¹¦ÄÜ£º
	´¥ÃþÆÁÌ½²âº¯Êý
	ÔÚ×¢²áÇý¶¯Ê±µ÷ÓÃ£¨ÒªÇó´æÔÚ¶ÔÓ¦µÄclient£©£»
	ÓÃÓÚIO,ÖÐ¶ÏµÈ×ÊÔ´ÉêÇë£»Éè±¸×¢²á£»´¥ÃþÆÁ³õÊ¼»¯µÈ¹¤×÷
²ÎÊý£º
	client£º´ýÇý¶¯µÄÉè±¸½á¹¹Ìå
	id£ºÉè±¸ID
return£º
	Ö´ÐÐ½á¹ûÂë£¬0±íÊ¾Õý³£Ö´ÐÐ
********************************************************/
